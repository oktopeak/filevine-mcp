...
const TOKEN_DIR = path.join(os.homedir(), ".oktopeak-filevine");
...
export async function saveTokens(tokens: FileVineTokens): Promise<void> {
...
export async function loadTokens(): Promise<FileVineTokens | null> {
  let combined: Buffer;
  try {
  ...
  try {
    ...
    return JSON.parse(decrypted.toString("utf8")) as FileVineTokens;
  } catch (err: unknown) {
    console.error(
